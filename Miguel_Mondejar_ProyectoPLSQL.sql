    ...
    OPEN INFO_POKEMON; -- Abrimos cursor
        ...
        EXIT WHEN INFO_POKEMON%NOTFOUND; -- Salimos si no hay más registros
           RETURN 'NOMBRE: ' || pok.NOMBRE || ' | REGIÓN ORIGEN: ' || pok.NOMBRE_REGION || ' | TIPO: ' || pok.NOMBRE_TIPOS || ' | NÚMERO POKÉDEX: ' || pok.COD_POKEDEX
            || ' | ATAQUE PRINCIPAL: ' || pok.NOMBRE_ATAQUE || ' | PESO y ALTURA: ' || pok.PESO || ' kg y ' || pok.ALTURA || ' m';
        END LOOP;
...
EXCEPTION -- Excepciones del programa por si ocurre algún error
    ...
    WHEN TOO_MANY_ROWS THEN DBMS_OUTPUT.PUT_LINE('Devuelve más de una fila');
